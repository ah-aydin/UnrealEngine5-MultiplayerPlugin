...
{
	// Check if online session interface is valid
	if (!OnlineSessionInterface.IsValid())
	{
		return;
	}

	// Destroy the existing session if there is any
	...
	else
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Red,
				FString(TEXT("Failed to craete session!"))
			);
		}
	}
}
			...
			if (MatchType == FString("FreeForAll"))
			{
				if (GEngine)
				{
					GEngine->AddOnScreenDebugMessage(
						...
						15.f,
						FColor::Cyan,
						FString(TEXT("Joining free for all match!"))
					);
				}
	...
	else
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Red,
				FString(TEXT("Failed to find sessions"))
			);
		}
	}
}
